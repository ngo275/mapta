#!/usr/bin/env python3
"""
Test script to verify E2B sandbox setup and network connectivity.

This script tests the complete E2B integration to ensure MAPTA can
...
from e2b_sandbox_factory import test_network_connectivity
...
def test_sandbox_creation():
    ...
    if sandbox is None:
        ...
        print("Check that SANDBOX_FACTORY and E2B_API_KEY are set correctly")
        return False
...
def test_basic_commands(sandbox):
    ...
    try:
        result = sandbox.commands.run("echo 'Hello from sandbox'", timeout=10)
...
def test_mapta_integration():
    ...
    if sandbox_factory != "e2b_sandbox_factory:create_e2b_sandbox":
        print(f"✗ SANDBOX_FACTORY not set correctly: {sandbox_factory}")
        print("Expected: e2b_sandbox_factory:create_e2b_sandbox")
        return False
...
def main():
    """Run all tests to verify E2B setup."""
    logging.basicConfig(level=logging.INFO)
    
    print("=" * 50)
    print("MAPTA E2B Sandbox Setup Verification")
    print("=" * 50)
    
    if not test_mapta_integration():
        print("\n❌ Environment configuration failed")
        print("Please check E2B_SETUP.md for configuration instructions")
        return False
